...
function CardPost(props) {
    ...
    return (
        // CardPost
        <div key={post._id} dir='ltr' className='flex flex-col items-start  gap-4 text-right  rounded-md w-full h-fit px-4 py-6  border border-solid border-gray-300 shadow-sm bg-zinc-100 dark:bg-slate-700  dark:border-none dark:text-white font-["Vazir"] ' >
            <div className='w-full flex flex-col md:flex-row gap-6  flex-wrap   items-center justify-between' >
                <div className='flex flex-col md:flex-row w-full items-center gap-3'>
                    <a href={`/user/dashboard/${post.user._id}`} className='h-fit w-fit' >
                        {
                            post.user.avatar !== null ?
                            <img src={`${process.env.REACT_APP_API_URL}/${post.user.avatarpath}`} alt="user" className='h-16 w-16 rounded-full ring-2 ring-green-200  flex-shrink-0 object-cover self-center shadow-md hover:scale-105 transition-all' />
            ...
            {
                isSinglePage ?
                <h3 className='text-xl h-fit font-[600] text-gray-900 my-3 dark:text-gray-50 hover:text-opacity-85 ' >{post.title}</h3>
                :
...

            {
                ...
                <a href={`/posts/${post.slug}`} className='w-full' >
                    <p  id={post._id}  dir='ltr' className='font-[400] font-["Vazir"] line-clamp-3 overflow-hidden text-ellipsis h-fit   text-center md:text-left text-gray-800 text-lg leading-8 dark:text-gray-50 hover:text-opacity-85 dark:bg-slate-800 px-2 py-1 rounded-md  '  ></p>
                </a>
            }
            <div className='flex items-center mt-4 flex-wrap gap-x-2 gap-y-4 w-full text-sm md:text-lg' >
            {
                post.tags.map(tag =>{
                    return (
                        <span key={tag._id} className=' bg-teal-700 dark:shadow-lg text-sm dark:shadow-teal-500/50 p-2  rounded-md text-white font-["Nunito"]' >
