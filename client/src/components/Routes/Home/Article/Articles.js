      ...
      NodejsApi.delete(`/like/${id}/${kind}/${single}/${moreData}`)
        .then((response) => {
          ...
          if (response.data.success) {
            ...
            if (
              articles.some((article) => article._id === dislikedArticle._id)
            ) {
              let index = articles.findIndex(
                (element) => element._id === dislikedArticle._id
              );
            ...
            if (
              weekFavouriteArticles.some(
                (weekFavouriteArticle) =>
                  weekFavouriteArticle._id === dislikedArticle._id
              )
            ) {
              let index = weekFavouriteArticles.findIndex(
                (element) => element._id === dislikedArticle._id
              );

              weekFavouriteArticles[index].likedByThisUser = false;
              weekFavouriteArticles[index].likeCount =
                dislikedArticle.likeCount - 1;
        ...
        .then((response) => {
          ...
          if (response.data.success) {
            ...
            if (articles.some((article) => article._id === likedArticle._id)) {
              let index = articles.findIndex(
                (element) => element._id === likedArticle._id
              );
            ...
            if (
              weekFavouriteArticles.some(
                (weekFavouriteArticle) =>
                  weekFavouriteArticle._id === likedArticle._id
              )
            ) {
              let index = weekFavouriteArticles.findIndex(
                (element) => element._id === likedArticle._id
      ...
      NodejsApi.delete(`/save/${id}/${kind}/${single}/${moreData}`)
        .then((response) => {
          ...
          if (response.data.success) {
            ...
            if (
              articles.some((article) => article._id === unsavedArticle._id)
            ) {
              let index = articles.findIndex(
                (element) => element._id === unsavedArticle._id
              );
            ...
            if (
              weekFavouriteArticles.some(
                (weekFavouriteArticle) =>
                  weekFavouriteArticle._id === unsavedArticle._id
              )
            ) {
              let index = weekFavouriteArticles.findIndex(
                (element) => element._id === unsavedArticle._id
              ...
              weekFavouriteArticles[index].saveCount =
                unsavedArticle.saveCount - 1;
        ...
        .then((response) => {
          ...
          if (response.data.success) {
            ...
            if (articles.some((article) => article._id === savedArticle._id)) {
              let index = articles.findIndex(
                (element) => element._id === savedArticle._id
              );
            ...
            if (
              weekFavouriteArticles.some(
                (weekFavouriteArticle) =>
                  weekFavouriteArticle._id === savedArticle._id
              )
            ) {
              let index = weekFavouriteArticles.findIndex(
                (element) => element._id === savedArticle._id
        ...
        <div className="flex flex-col w-full h-fit">
          ...
          <main className='flex flex-col items-center justify-center md:flex-row md:flex-start w-full h-fit min-h-screen gap-10 font-["Nunito"] bg-gradient-to-br from-30% from-slate-100 to-70% to-slate-300  dark:bg-gradient-to-t dark:from-10% dark:from-slate-600 dark:to-90% dark:to-slate-800 '>
            ...
            <div className="hidden lg:flex flex-col lg:w-[230px] xl:w-[15%] w-[15%] dark:text-white ">
                <CardTagFilter isAuthenticated={userstate.isAuthenticated} tags={tags} userTags={userTags} />
            </div>
