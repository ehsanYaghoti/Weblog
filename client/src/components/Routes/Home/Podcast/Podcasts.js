...
function Podcasts(props) {
    ...
    useEffect(() => {
        ...
        NodejsApi.get('http://localhost:5000/podcasts')
        .then(response => {
    ...
    useEffect(() => {
        ...
        NodejsApi.get(`http://localhost:5000/podcasts?category=${searchParams.get('category')}`)
        .then(response => {
            if(! response.data.success){
                if(response.data.code === 204){
                    return setNocontent({
    ...
    let paginationHandler = (e) => {
        ...
        NodejsApi.get(`http://localhost:5000/podcasts?${name}=${value}`)
        .then(response => {
            if(! response.data.success){
                // setLoading(false)
                if(response.data.code === 204){
    ...
    let sortHandler = (e) => {
        ...
        NodejsApi.get(`http://localhost:5000/podcasts?${name}=${queries[name]}`)
        .then(response => {
            if(! response.data.success){
                // setLoading(false)
                return  setSuccess(prevState => {
