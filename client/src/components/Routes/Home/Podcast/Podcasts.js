...
import CardTagFilter from "src/components/Layouts/Home/Cards/CardTagFilter";
      ...
      NodejsApi.delete(`/like/${id}/${kind}/${single}/${moreData}`)
        .then((response) => {
          ...
          if (response.data.success) {
            ...
            if (
              podcasts.some((podcast) => podcast._id === dislikedPodcast._id)
            ) {
              let index = podcasts.findIndex(
                (element) => element._id === dislikedPodcast._id
              );
            ...
            if (
              weekFavouritePodcasts.some(
                (weekFavouritePodcast) =>
                  weekFavouritePodcast._id === dislikedPodcast._id
              )
            ) {
              let index = weekFavouritePodcasts.findIndex(
                (element) => element._id === dislikedPodcast._id
              );

              weekFavouritePodcasts[index].likedByThisUser = false;
              weekFavouritePodcasts[index].likeCount =
                dislikedPodcast.likeCount - 1;
        ...
        .then((response) => {
          ...
          if (response.data.success) {
            ...
            if (podcasts.some((podcast) => podcast._id === likedPodcast._id)) {
              let index = podcasts.findIndex(
                (element) => element._id === likedPodcast._id
              );
            ...
            if (
              weekFavouritePodcasts.some(
                (weekFavouritePodcast) =>
                  weekFavouritePodcast._id === likedPodcast._id
              )
            ) {
              let index = weekFavouritePodcasts.findIndex(
                (element) => element._id === likedPodcast._id
      ...
      NodejsApi.delete(`/save/${id}/${kind}/${single}/${moreData}`)
        .then((response) => {
          ...
          if (response.data.success) {
            ...
            if (
              podcasts.some((podcast) => podcast._id === unsavedPodcast._id)
            ) {
              let index = podcasts.findIndex(
                (element) => element._id === unsavedPodcast._id
              );
            ...
            if (
              weekFavouritePodcasts.some(
                (weekFavouritePodcast) =>
                  weekFavouritePodcast._id === unsavedPodcast._id
              )
            ) {
              let index = weekFavouritePodcasts.findIndex(
                (element) => element._id === unsavedPodcast._id
              ...
              weekFavouritePodcasts[index].saveCount =
                unsavedPodcast.saveCount - 1;
        ...
        .then((response) => {
          ...
          if (response.data.success) {
            ...
            if (podcasts.some((podcast) => podcast._id === savedPodcast._id)) {
              let index = podcasts.findIndex(
                (element) => element._id === savedPodcast._id
              );
            ...
            if (
              weekFavouritePodcasts.some(
                (weekFavouritePodcast) =>
                  weekFavouritePodcast._id === savedPodcast._id
              )
            ) {
              let index = weekFavouritePodcasts.findIndex(
                (element) => element._id === savedPodcast._id
          ...
          <main className='flex flex-col items-center justify-center md:flex-row w-full h-fit min-h-screen gap-10 font-["Nunito"]  bg-gradient-to-br from-30% from-slate-100 to-70% to-slate-300  dark:bg-gradient-to-t dark:from-10% dark:from-slate-600 dark:to-90% dark:to-slate-800 '>
            ...
            <div className="flex flex-col items-center w-full md:w-[85%]  h-fit px-1 py-6 lg:p-6 text-gray-800">
                ...
                <>
                  ...
                  <h2 className=" h-fit flex items-center self-start text-gray-900 dark:text-gray-50 mb-10  gap-2">
                    ...
                    <span className="text-2xl font-[600]  ">
                      week Favourite Podcasts
                    </span>
                  </h2>
                  {/* contents */}
                  {!noContent.state ? (
                    <span className="text-gray-500 font-[600] text-xl dark:text-white">
                      {noContent.message}
                    </span>
                  ) : (
              ...
              <h2 className=" h-fit flex items-center self-start my-10 mx-2  gap-2">
                <FontAwesomeIcon
                  icon={faArchive}
                  className="text-xl dark:text-gray-50"
                />
                <span className="text-2xl font-[600] text-gray-900 dark:text-gray-50">
                  All Podcasts
                </span>
              </h2>
              ...
              <div className="hidden md:flex flex-col items-center w-full ">
                ...
                <div className="flex self-start justify-center w-full min-w-full  rounded-md border border-solid border-gray-400  p-4 mb-10 bg-white dark:bg-teal-900 dark:border-none dark:text-white ">
                  <div className="flex items-center gap-1 md:gap-2 lg:gap-8 text-lg font-semibold ">
                    ...
                    <button
                      name="likeCount"
                      value={queries.likeCount}
                      onClick={sortHandler}
                      className="cursor-pointer hover:opacity-80 hover:bg-teal-600 hover:text-white focus:bg-teal-300 focus:text-white focus:ring-2 focus:ring-teal-900 rounded-3xl px-4 py-2"
                    >
                      likes
                    </button>
                    <button
                      name="saveCount"
                      value={queries.saveCount}
                      onClick={sortHandler}
                      className="cursor-pointer hover:opacity-80 hover:bg-teal-600 hover:text-white focus:bg-teal-300 focus:text-white focus:ring-2 focus:ring-teal-900 rounded-3xl px-4 py-2"
                    >
                      saves
                ...
                <div className="flex self-start justify-center w-full rounded-md border border-solid border-gray-400  p-4 mb-10 bg-white dark:bg-teal-900 dark:border-none dark:text-white">
                  <div className="flex items-center gap-8 text-lg font-semibold ">
                    <label className="flex items-center gap-2">
                      ...
                      <select
                        className="select dark:text-gray-500 outline-none border-none rounded-md w-32 "
                        name="createdAt"
                ...
                <div className="flex flex-col items-center gap-6 h-fit w-full py-8 ">
                  ...
                  >
                    <button
                      type="button"
                      id="dropBtn"
                      className="text-lg text-gray-600 dark:bg-slate-800 dark:text-gray-50 hover:bg-opacity-30 focus:bg-opacity-10 font-semibold h-20 py-6 w-full flex items-center justify-center gap-2 text-center cursor-pointer rounded-md border border-solid border-gray-300"
                      // onBlur={e => document.getElementById('sortDropdown').classList.replace('flex' , 'hidden')}
                      onClick={(e) =>
                        document
                    ...
                    >
                      <button
                        ...
                        value={queries.likeCount}
                        onClick={sortHandler}
                        className="cursor-pointer hover:opacity-80 hover:bg-teal-400 hover:text-white focus:bg-teal-100 focus:text-white focus:ring-2 focus:ring-teal-900 px-4 py-3"
                      >
                        likes
                      </button>
                      <button
                  ...
                  <div className="text-gray-600 dark:bg-slate-800 flex self-start items-center whitespace-nowrap justify-center w-full  rounded-md border border-solid border-gray-300  p-4 mb-10 dark:bg-slate-700/80 dark:text-gray-50">
                    <div className="flex items-center justify-center h-fit gap-8 text-lg font-semibold ">
                      <label className="h-fit flex items-center gap-2">
                        ...
                        <span>Filter By Time :</span>
                        <select
                          className="select dark:text-gray-600 outline-none rounded-sm"
                          name="createdAt"
                          onChange={paginationHandler}
                        >
              ...
              {/* contents */}
              {!noContent.state ? (
                <span className="text-gray-500 font-[600] text-xl min-h-screen dark:text-white">
                  {noContent.message}
                </span>
              ) : (
                <>
                  ...
                  {!(pagination.totalPages === 1) ? (
                    <PaginationContext.Provider
                      value={{ pagination, paginationHandler }}
                    >
                      <Pagination />
                    </PaginationContext.Provider>
                  ) : (
