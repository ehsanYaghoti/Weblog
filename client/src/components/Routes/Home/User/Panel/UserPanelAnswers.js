...
import CardAnswer from 'src/components/Layouts/Home/Cards/CardAnswer';
...
function UserPanelAnswers(props){
                    ...
                    <main className='flex flex-row items-center w-full h-fit min-h-screen gap-10     pl-0 font-["Nunito"]  bg-gradient-to-br from-30% from-slate-100 to-70% to-slate-300 dark:bg-gradient-to-t dark:from-10% dark:from-slate-600 dark:to-90% dark:to-slate-800 dark:text-gray-50' >
                        ...
                        (
                            ...
                            <h2 className=' h-fit flex items-center self-start my-10  gap-2' >
                                <span className='text-2xl font-[600] text-gray-900 dark:text-gray-50 ' >Answers</span>
                            </h2>
                            {
                            ! noContent.state ? 
                            <span className='' >{noContent.message}</span>
                            : ( 
                            <div className='grid justify-items-center grid-cols-1  pb-10 gap-y-9 gap-x-6 py-6 w-full h-fit '  >
                                {
                                    userstate.user.answers.map(answer => {
                                        return (
                                            <CardAnswer key={answer._id} answer={answer} userstate={authenticatedUser}  />
                                        )
                                    })
                                }
                            </div>
...
export default isAuthenticated(UserPanelAnswers , 'panel'); 